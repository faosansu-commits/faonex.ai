	...
	>
		<div class="mb-6 flex items-center gap-3">
			...
			<div>
				...
				<p class="mt-1 text-xs text-slate-500">{$t('register.subtitle')}</p>
			</div>
		</div>
		...
		<label class="mb-3 block text-sm">
			<span class="mb-1 block text-slate-400">{$t('register.username')}</span>
			<input
				bind:value={username}
				required
		...
		<label class="mb-3 block text-sm">
			<span class="mb-1 block text-slate-400">{$t('register.displayName')}</span>
			<input
		...
		<label class="mb-3 block text-sm">
			<span class="mb-1 block text-slate-400">{$t('register.password')}</span>
			<input
				type="password"
				bind:value={password}
				required
		...
		<label class="mb-6 block text-sm">
			<span class="mb-1 block text-slate-400">{$t('register.confirmPassword')}</span>
			<input
				type="password"
		...
		<p class="mt-4 text-center text-sm text-slate-500">
			{$t('register.hasAccount')} <a href="/login" class="font-medium text-violet-400 hover:text-violet-300">{$t('register.loginLink')}</a>
		</p>
	</form>
</div>
